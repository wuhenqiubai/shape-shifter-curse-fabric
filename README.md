...
| Phase 3A | 简单 Power（~25 个，仅构造器逻辑） | ✅ 完成 |
| Phase 3B | 中等 Power（~20 个，含 serverTick） | ✅ 完成 |
| Phase 3C | 复杂 Power（~15 个，嵌套类/多字段） | ✅ 完成 |
| Phase 4 | Action 迁移（13 个文件 → EntityActionType） | ✅ 完成 |
| Phase 5 | Condition 迁移（9 个文件 → EntityConditionType/ItemConditionType） | ✅ 完成 |
| Phase 6 | 编译验证 & 功能测试 | 🔄 剩余 ~100 个错误 |
...
| 类别 | 文件数 | 预估工时 | 说明 |
|------|--------|---------|------|
| 🔴 Advancement 触发条件 | 17 个文件 | ~2~3h | `PowerType<?>` 泛型、`AbstractCriterion` 1.21.1 改名 |
| 🟡 Origins 旧 API 残留 | 8 个文件 | ~1~2h | `PowerTypeRegistry`、`ConditionTypes`、Badge 相关 |
| 🟠 Satin 2.0 API 变更 | 1 个文件 | ~0.5h | `ShaderEffectRenderCallback` 等包路径更新 |
| 🟢 Minecraft 1.21.1 API 变更 | 散落各处 | ~0.5h | `FabricItemSettings`、`Instrument` 等 |
