...
| ✅ | Client 入口 | `ShapeShifterCurseFabricClient` 恢复 (部分 Renderer/Feature stub) |
...
| 🚧 | Recipe/Brewing | `Recipe` final class，BrewingRecipeUtils/BrewingRecipeRegistryMixin 禁用 |
| 🚧 | Origins 集成 | Origin/OriginLayer 部分适配，~10 子模块文件需 Calio API 深度迁移 |
...
> **编译状态**: ~50 错误，集中在 Origins 集成模块 (Calio `SerializableDataType` API)
