...
>
> **基于主线commit
**: [660d2f5](https://github.com/onixary/shape-shifter-curse-fabric/commit/660d2f5efe58bb309a354664495981984a56121d)
...
- switch case 穿透：`handleMoonEndTransform()` 中 case 2 缺少 `break`，靠空 case 才正常工作
- `Thread.sleep` 在渲染线程：`executeClientFirstPersonReset()` 用裸线程 + sleep 修改 FPM 配置

**P1 — 健壮性**

- 盔甲槽硬编码 9 个物品 ID → 运行时解析标签 JSON（`MorphScaleTagLoader`），数据包可扩展
- `TickManager.tickServerAll()` 在玩家循环内重复调用 → 移到循环外
- `ModPacketsS2C` 中第二个 `Thread.sleep` 轮询 → `ClientTicker`

**P2 — 代码质量**

- 删除废弃的 OWO 旧版 UI（`BookOfShapeShifterScreen` / `StartBookScreen`）及反射代码
- 删除 ~110 行注释掉的死代码和 `clearMoonEndFlags()` 死方法
- 形态索引 -2/-1/0/1/2/3/5 → `PlayerFormPhase.INDEX_*` 命名常量
...
- 核心游戏流程已趋于稳定，P0/P1/P2 关键 bug 已修复
...
| Claude Code (Anthropic)                       | 代码审查与重构辅助 / Code Review & Refactoring |
