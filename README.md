...
| 文件 | 类别 | 原因 |
|------|------|------|
| `TransformativeEntitySpawning.java.bak` | Entity | SpawnRestriction API 移除 |
| `OriginCommand.java.bak` | Origins | Command API 变更 |
| `WaterProtectionEnchantment.java.bak` | Origins | 已改为 JSON 数据文件 |
| `AutoBadgeCallback.java.bak` | Origins | Calio API 变更 |
| `ModPacketsS2C.java.bak` | Origins | 网络层已重写 |
| `ViewOriginScreen.java.bak` | Origins | Screen API 变更 |
| `CraftingRecipeTooltipComponent.java.bak` | Origins | 渲染 API 变更 |
| `OriginsRegistryMixin.java.bak` | Origins | Mixin target 变更 |
| `BrewingRecipeRegistryMixin.java.bak` | Recipe | Recipe final class |
| `BrewingRecipeReloadListener.java.bak` | Recipe | BrewingRecipeUtils 依赖 |
| `BrewingRecipeUtils.java.bak` | Recipe | Recipe API 完全重写 |
| `VirtualTotemMixin.java.bak` | Power | Potion API 变更 |
| `AppleSkin.java.bak` | Integration | AppleSkin mod API 变更 |
| `IronGolemEntityMixin.java.bak` | Mixin | Entity API 变更 |
| `ScareSkeletonMixin.java.bak` | Mixin | Entity API 变更 |
| `SpiderEntityMixin.java.bak` | Mixin | Entity API 变更 |

### 源码中注释/临时替代的功能

| 文件 | 功能 | 状态 |
|------|------|------|
| `LivingEntityMixin.java:98` | 女巫掉落狐狸诅咒药水 | `PotionContentsComponent(RegistryEntry<Potion>)` — 待 Registry API 查询 |
| `WitchEntityMixin.java:47` | 女巫投掷自定义药水 | 同上，`ItemStack.EMPTY` 临时替代 |
