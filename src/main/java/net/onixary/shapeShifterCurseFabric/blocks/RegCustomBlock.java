...
public final class RegCustomBlock {
    ...
    public static final Block WEB_COMPOSTER = register("web_composter", new WebComposterBlock(AbstractBlock.Settings.create().mapColor(MapColor.WHITE_GRAY).instrument(NoteBlockInstrument.BASS).strength(0.6F).sounds(BlockSoundGroup.WOOL).burnable().nonOpaque()));
    public static final Block DEW_COVERED_COBWEB = register("dew_covered_cobweb", new DewCoveredCobwebBlock(AbstractBlock.Settings.create().mapColor(MapColor.WHITE_GRAY).instrument(NoteBlockInstrument.BELL).strength(1.0F).sounds(BlockSoundGroup.WOOL).noCollision().nonOpaque()));
    ...
    private static <T extends Block> T registerWithOutItem(String path, T block) {
        Registry.register(Registries.BLOCK, Identifier.of(ShapeShifterCurseFabric.MOD_ID, path), block);
        return block;
    }

    private static <T extends Block> T register(String path, T block) {
        Registry.register(Registries.BLOCK, Identifier.of(ShapeShifterCurseFabric.MOD_ID, path), block);
        Registry.register(Registries.ITEM, Identifier.of(ShapeShifterCurseFabric.MOD_ID, path), new BlockItem(block, new Item.Settings()));
        return block;
    }
